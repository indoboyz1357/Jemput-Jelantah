          ...
          return (
            <div key={bill.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${typeConfig.color}`}>
                  {typeConfig.label}
                </span>
              </div>
              ...
              {isEditing ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4">
                  <div className="bg-blue-50 rounded-lg p-3">
                    <div className="text-xs sm:text-sm text-blue-600 mb-1">Liter</div>
                    <input
                      type="number"
                      value={editForm.liters}
                      onChange={(e) => setEditForm({...editForm, liters: e.target.value})}
                      className="w-full px-2 py-1 border border-blue-300 rounded text-xs sm:text-sm"
                      min="1"
                    />
                  </div>
                  <div className="bg-blue-50 rounded-lg p-3">
                    <div className="text-xs sm:text-sm text-blue-600 mb-1">Rate</div>
                    <input
                      type="number"
                      value={editForm.rate}
                  ...
                  <div className="bg-blue-50 rounded-lg p-3">
                    <div className="text-xs sm:text-sm text-blue-600 mb-1">Status</div>
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        className="rounded"
                      />
                      <span className="text-xs sm:text-sm">Lunas</span>
                    </label>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-4">
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="text-xs sm:text-sm text-gray-600">Liter</div>
                    <div className="text-base sm:text-lg font-semibold text-gray-900">{bill.liters}L</div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <div className={`text-base sm:text-lg font-semibold ${bill.paid ? 'text-green-600' : 'text-red-600'}`}>
              ...
              {bill.paymentProofUrl && (
                <div className="mb-4 p-3 bg-green-50 rounded-lg border border-green-200">
                  <div className="flex items-center gap-2 mb-2">
                    <Image className="text-green-600" size={16} />
                    <span className="text-xs sm:text-sm font-medium text-green-800">Bukti Pembayaran</span>
                  </div>
                  <img 
                ...
                {isEditing ? (
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-none bg-green-500 text-white px-3 py-2 rounded-lg hover:bg-green-600 transition-colors duration-200 flex items-center justify-center gap-1 text-sm"
                    >
                      <Save size={16} />
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-none bg-gray-500 text-white px-3 py-2 rounded-lg hover:bg-gray-600 transition-colors duration-200 flex items-center justify-center gap-1 text-sm"
                    >
                      <X size={16} />
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <button
                      ...
                      className="bg-blue-500 text-white px-3 py-2 rounded-lg hover:bg-blue-600 transition-colors duration-200 flex items-center justify-center gap-1 text-sm"
                    >
                      <Edit2 size={16} />
                      ...
                      <>
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              handleUploadPaymentProof(bill.id, file);
                            }
                          }}
                          className="hidden"
                          id={`payment-${bill.id}`}
                        />
                        <label
                          htmlFor={`payment-${bill.id}`}
                          className={`flex items-center justify-center gap-1 px-3 py-2 rounded-lg transition-colors duration-200 cursor-pointer text-sm ${
                            uploadingPayment === bill.id
                        ...
                        >
                          ...
                          <span className="hidden sm:inline">
                            {uploadingPayment === bill.id ? 'Uploading...' : 'Upload Bukti Bayar'}
                          </span>
                          <span className="sm:hidden">
                            {uploadingPayment === bill.id ? 'Upload...' : 'Bukti'}
                          </span>
                        </label>
                      </>
                    )}
